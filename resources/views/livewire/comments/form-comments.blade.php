<div>
    ...
    <form wire:submit.prevent="save" class="space-y-5" x-data="{
        ...
        <div x-on:captcha-token-received.window="$wire.save($event.detail.token)">
            <button type="button" @click="iniciarEnvio()"
                ...
                <span wire:loading>{{ __('reviws_form_button_1') }}</span>
            </button>
        </div>
    </form>
</div>
